...
class XLSXFormat:
    ...
    @classmethod
    def export_set(cls, DataTable, freeze_panes=True):
        """Returns XLSX representation of DataTable."""
        wb = Workbook()
        ws = wb.worksheets[0]
        ws.title = DataTable.title if DataTable.title else 'Tablib DataTable'

        cls.dset_sheet(DataTable, ws, freeze_panes=freeze_panes)
    ...
    @classmethod
    def export_book(cls, DataSet, freeze_panes=True):
    ...
    @classmethod
    def dset_sheet(cls, DataTable, ws, freeze_panes=True):
