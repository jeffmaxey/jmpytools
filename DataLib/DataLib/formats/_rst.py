...
def _max_word_len(text):
    ...
    >>> _max_word_len('Python Module for Tabular DataTables')
    8
    """
...
class ReSTFormat:
    ...
    @classmethod
    def _get_column_string_lengths(cls, DataTable):
        """
        Returns a list of string lengths of each column, and a list of
        maximum word lengths.
        """
        if DataTable.headers:
            column_lengths = [[len(h)] for h in DataTable.headers]
            word_lens = [_max_word_len(h) for h in DataTable.headers]
        else:
            column_lengths = [[] for _ in range(DataTable.width)]
            word_lens = [0 for _ in range(DataTable.width)]
        for row in DataTable.dict:
    ...
    @classmethod
    def _get_column_widths(cls, DataTable, max_table_width=MAX_TABLE_WIDTH, pad_len=3):
        """
        Returns a list of column widths proportional to the median length
        of the text in their cells.
        """
        str_lens, word_lens = cls._get_column_string_lengths(DataTable)
    ...
    @classmethod
    def export_set_as_simple_table(cls, DataTable, column_widths=None):
        """
        Returns reStructuredText grid table representation of DataTable.
        """
        lines = []
        wrapper = TextWrapper()
        if column_widths is None:
            column_widths = cls._get_column_widths(DataTable, pad_len=2)
    ...
    @classmethod
    def export_set_as_grid_table(cls, DataTable, column_widths=None):
        """
        Returns reStructuredText grid table representation of DataTable.


        >>> from tablib import DataTable
        ...
        wrapper = TextWrapper()
        if column_widths is None:
            column_widths = cls._get_column_widths(DataTable)
    ...
    def export_set(cls, DataTable, **kwargs):
        """
        Returns reStructuredText table representation of DataTable.
        ...
        """
        if not DataTable.dict:
            return ''
        force_grid = kwargs.get('force_grid', False)
        max_table_width = kwargs.get('max_table_width', cls.MAX_TABLE_WIDTH)
        column_widths = cls._get_column_widths(DataTable, max_table_width)

        use_simple_table = cls._use_simple_table(
            DataTable.headers[0] if DataTable.headers else None,
            DataTable.get_col(0),
            column_widths[0],
        )
        if use_simple_table and not force_grid:
            return cls.export_set_as_simple_table(DataTable, column_widths)
        else:
            return cls.export_set_as_grid_table(DataTable, column_widths)

    @classmethod
    def export_book(cls, DataSet):
        """
        reStructuredText representation of a DataSet.

        Tables are separated by a blank line. All tables use the grid
        format.
        """
        return '\n\n'.join(cls.export_set(DataTable, force_grid=True)
                           for DataTable in DataSet._DataTables)
