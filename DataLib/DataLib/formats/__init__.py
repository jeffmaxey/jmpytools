...
class Registry:
    ...
    def register(self, key, format_or_path):
        from tablib.core import DataSet, DataTable

        # Create DataSet.<format> read or read/write properties
        setattr(DataSet, key, ImportExportBookDescriptor(key, format_or_path))

        # Create DataTable.<format> read or read/write properties,
        # and DataTable.get_<format>/set_<format> methods.
        setattr(DataTable, key, ImportExportSetDescriptor(key, format_or_path))
        try:
            setattr(DataTable, 'get_%s' % key, partialmethod(DataTable._get_in_format, key))
            setattr(DataTable, 'set_%s' % key, partialmethod(DataTable._set_in_format, key))
